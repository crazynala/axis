...
function isNavLinkItem(item: NavMenuItem): item is NavLinkItem {
  return !isNavDividerItem(item) && !isNavGroupItem(item);
}
...
function AppShellLayout({
  ...
  useEffect(() => {
    const onStorage = (e: StorageEvent) => {
      if (!e.key) return;
      if (!e.key.startsWith("axis:nl:v1:")) return;
      setNavRefresh((v) => v + 1);
    };
  ...
  const navHrefByBase = useMemo(() => {
    const map = new Map<string, string>();
    const items = [
      ...navTopItems.flatMap((item) =>
        isNavGroupItem(item) ? [item.parent, ...item.children] : [item]
      ),
      ...navBottomItems,
    ].filter((item) => isNavLinkItem(item)) as NavLinkItem[];
    for (const item of items) {
      let href = item.to;
      const saved = getSavedNavLocation(item.to);
      if (saved) href = saved;
      else if (navHydrated) {
        const search = getSavedIndexSearch(item.to);
        if (search) href = `${item.to}${search}`;
      }
      map.set(item.to, href);
    }
    return map;
  }, [navBottomItems, navHydrated, navTopItems, navRefresh]);

  // Color scheme toggle moved to settings page
  const getNavLinkConfig = (
    item: NavLinkItem,
    opts?: { insideModule?: boolean }
  ) => {
    let href = navHrefByBase.get(item.to) || item.to;
    const insideModule =
      opts?.insideModule ??
  ...
  const navLinkStyles = {
    root: { textDecoration: "none", color: "inherit", overflow: "visible" },
    body: { overflow: "visible " }, // for accordion carets
    label: { textDecoration: "none", overflow: "visible" },
  } as const;

  const renderNavLinkItem = (
    item: NavLinkItem,
    opts?: {
    ...
    const { href, onClick, insideModule } = getNavLinkConfig(item, {
      insideModule: opts?.insideModule,
    });
    if (desktopNavOpened) {
      return (
        <NavLink
          ...
          rightSection={opts?.rightSection}
          key={item.to}
          onClick={onClick}
          styles={navLinkStyles}
          fw={insideModule ? 600 : 400}
        />
      );
    }
    return (
      <Tooltip label={item.label} position="right" withArrow>
        <NavLink
          px="xs"
          component={RemixNavLink}
          label={opts?.labelOverride ?? item.icon}
          to={href}
          key={item.to}
          onClick={onClick}
          styles={navLinkStyles}
  ...
  const renderGroupParent = (group: NavGroupItem) => {
    ...
    return renderNavLinkItem(group.parent, {
      insideModule: active,
      rightSection: caret,
    });
  };
  const renderCollapsedGroupParent = (group: NavGroupItem) => {
    const active = isGroupActive(group);
    const CaretIcon = active ? IconChevronDown : IconChevronRight;
    const label = (
      <div
        style={{
          position: "relative",
            ...
            {desktopNavOpened
              ? navTopItems.map((item) => {
                  ...
                  if (isNavGroupItem(item)) {
                    ...
                    return (
                      <div key={item.key}>
                        {renderGroupParent(item)}
                        {active ? (
                          <Stack gap={0} pl="md">
                            {item.children.map((child) =>
                              renderNavLinkItem(child)
                            )}
                          </Stack>
                        ) : null}
                      </div>
              ...
              : navTopItems.map((item) => {
                  ...
                  if (isNavGroupItem(item)) {
                    ...
                    return (
                      <div key={item.key}>
                        {renderCollapsedGroupParent(item)}
                        {active ? (
                          <Stack gap={0}>
                            {item.children.map((child) =>
                              renderNavLinkItem(child)
