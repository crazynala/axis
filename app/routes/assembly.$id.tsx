...
export async function loader({ params }: LoaderFunctionArgs) {
  ...
  const assembly = await prisma.assembly.findUnique({ where: { id }, select: { id: true, jobId: true } });
...
export async function action({ params }: ActionFunctionArgs) {
  ...
  if (assembly.jobId) return redirect(`/jobs/${assembly.jobId}/assembly/${assembly.id}`);
  return redirect(`/assembly`);
}
