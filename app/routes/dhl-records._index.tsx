...
export async function loader(args: LoaderFunctionArgs) {
  // Redirect top-level path to admin location to avoid duplicate entry points
  const url = new URL(args.request.url);
  if (url.pathname === "/dhl-records") {
...
export default function DHLRecordsIndexRoute() {
    ...
    <div>
      ...
      <DataTable
        ...
        columns={[
          {
            accessor: "id",
            render: (r: any) => <Link to={`/dhl-records/${r.id}`}>{r.id}</Link>,
          },
          {
