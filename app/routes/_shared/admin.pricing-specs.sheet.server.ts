...
export async function loadPricingSpecEdit({
  params,
  request,
}: LoaderFunctionArgs) {
...
export async function savePricingSpecEdit({
  params,
  request,
}: ActionFunctionArgs) {
  await requireAdminUser(request);
...
export async function loadPricingSpecNew({ request }: LoaderFunctionArgs) {
  await requireAdminUser(request);
  return json({
    rows: [],
...
export async function savePricingSpecNew({ request }: ActionFunctionArgs) {
  await requireAdminUser(request);
  const bodyText = await request.text();
  let payload: any = null;
  try {
