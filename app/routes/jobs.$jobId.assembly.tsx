...
export async function loader({ params }: LoaderFunctionArgs) {
  ...
  const job = await prisma.job.findUnique({ where: { id: jobId }, include: { assemblies: { select: { id: true, name: true, status: true }, orderBy: { id: "asc" } } } });
  if (!job) throw new Response("Not Found", { status: 404 });
  return json({ job: { id: job.id, name: job.name }, assemblies: job.assemblies });
}
