...
export function ExpenseFindManager({
  ...
  const semanticKeys = useMemo(() => expenseSpec.find.deriveSemanticKeys(), []);
  ...
  const readLastFindQs = useCallback(() => {
    try {
      const qs = window.sessionStorage.getItem("axis:lastFind:expenses");
  ...
  const buildRestoreState = useCallback(
    (qs: string | null) => {
      ...
      if (q && !values.memo) values.memo = q;
      return {
