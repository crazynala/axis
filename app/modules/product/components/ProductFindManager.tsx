...
export function ProductFindManager({
  ...
  const semanticKeys = useMemo(() => {
    ...
    return productSpec.find.deriveSemanticKeys(metaFields);
  }, [filterableDefs, globalOptions?.productAttributeOptionsByDefinitionId]);
  ...
  const readLastFindQs = useCallback(() => {
    try {
      const qs = window.sessionStorage.getItem("axis:lastFind:products");
  ...
  const buildRestoreState = useCallback(
    (qs: string | null) => {
      ...
      if (q && !values.name) values.name = q;
      return {
