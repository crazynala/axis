...
export function ShipmentFindManager({
  ...
  const semanticKeys = useMemo(
    () => shipmentSpec.find.deriveSemanticKeys(),
    []
  );
  ...
  const readLastFindQs = useCallback(() => {
    try {
      const qs = window.sessionStorage.getItem("axis:lastFind:shipments");
  ...
  const buildRestoreState = useCallback(
    (qs: string | null) => {
      ...
      if (q && !values.trackingNo) values.trackingNo = q;
      return {
