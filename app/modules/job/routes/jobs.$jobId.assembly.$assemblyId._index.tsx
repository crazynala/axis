...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  if (intent === "costing.refreshProduct") {
    const cid = Number(form.get("id"));
    if (Number.isFinite(cid)) {
      const costing = await prisma.costing.findUnique({
        where: { id: cid },
      ...
      if (costing?.flagDefinedInProduct && costing.productId) {
        ...
        const matchedLine =
          parentLines.find((ln) => Number(ln.childId) === costing.productId) ||
          null;
        ...
        const updateData: any = {
          ...
          manualSalePrice:
            childProduct?.manualSalePrice != null
              ? Number(childProduct.manualSalePrice)
              : null,
          manualMargin:
            childProduct?.manualMargin != null
              ? Number(childProduct.manualMargin)
              : null,
          externalStepType: childProduct?.externalStepType ?? null,
          leadTimeDays:
            childProduct?.leadTimeDays != null
              ? Number(childProduct.leadTimeDays)
              : null,
        };
        await prisma.costing.update({
          where: { id: cid },
